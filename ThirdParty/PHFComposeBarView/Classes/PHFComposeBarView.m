...
CGFloat const kHorizontalSpacing          =  2.0f;
...
CGFloat const kTextContainerRightPadding  =  8.0f;
CGFloat const kTextContainerTopPadding    =  8.0f;
...
CGFloat const kButtonHeight               = 44.0f;
CGFloat const kButtonTouchableOverlap     =  0.0f;
CGFloat const kButtonRightMargin          = 12.0f;
CGFloat const kButtonBottomMargin         =  0.0f;
CGFloat const kUtilityButtonWidth         = 40.0f;
CGFloat const kUtilityButtonHeight        = 44.0f;
CGFloat const kUtilityButtonBottomMargin  =  0.0f;
...
- (id)initWithFrame:(CGRect)frame withTextViewOffset:(CGFloat)textViewOffset{
    self = [super initWithFrame:frame];
    
    self.textViewOffset = textViewOffset;
    [self calculateRuntimeConstants];
    [self setup];
...
- (UIView *)topLineView {
    if (!_topLineView) {
        ...
        [_topLineView setBackgroundColor:[UIColor clearColor]];
...
- (UIToolbar *)backgroundView {
    if (!_backgroundView) {
        ...
        [_backgroundView setBarTintColor:[UIColor color6]];
        [_backgroundView setAutoresizingMask:UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight];
        
        
//        self.toolBarBackgroundImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"compose_bg"]];
...
//        self.toolBarBackgroundImage.frame = frame;
        ...
        frame = self.toolBarBackgroundImage.frame;
        frame.size.width = self.bounds.size.width - sendlabel.frame.size.width- 24 - 40;
        ...
        [_backgroundView addSubview:self.toolBarBackgroundImage];
    }
...
- (UIButton *)textContainer {
    if (!_textContainer) {
        ...
        CGRect textContainerFrame = CGRectMake(0,
                                               kTextContainerTopMargin,
                                               [self bounds].size.width -40 -[self button].bounds.size.width,
                                               self.bounds.size.height -kTextContainerTopMargin -kTextContainerBottomMargin);
        ...
        textViewFrame.size.height = textHeight; // -kTextContainerBottomMargin;
        
        [[self textView] setFrame:textViewFrame];
        [[self textView] setBackgroundColor:[UIColor clearColor]];
//        [[self textView] setBackgroundColor:[[UIColor redColor] colorWithAlphaComponent:0.5]];
...
- (UITextView *)textView {
    if (!_textView) {
        ...
        [_textView setFont:[UIFont fontWithName:@"STHeitiTC-Light" size:15]];
...
- (void)resizeTextViewIfNeededAnimated:(BOOL)animated {
    ...
    CGRect frame = self.toolBarBackgroundImage.frame;
...
- (void)resizeButton {
    ...
    // !!!: Set button frame here
    
    CGFloat buttonLabelWidth;
    for (UIView *subview in [[self button] subviews])
    {
        if ([subview isKindOfClass:[UILabel class]])
        {
            buttonLabelWidth = subview.bounds.size.width;
            break;
        }
    }
    ...
    // !!!: Set text container frame here
    
    //    textContainerFrame.size.width -= widthDelta;
    CGRect frame = [[self textContainer] frame];
    // textContainer will shift and resize by one utility button @ - (void)shifTextFieldInDirection:(NSInteger)direction
    frame.size.width = self.bounds.size.width -[self button].bounds.size.width + self.textViewOffset;
    frame.origin.x = -self.textViewOffset;
    //    frame.origin.x = [self utilityButton].frame.origin.x +[self utilityButton].bounds.size.width;
    [[self textContainer] setFrame:frame];
